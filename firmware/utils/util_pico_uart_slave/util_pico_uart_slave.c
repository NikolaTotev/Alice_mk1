...
void on_uart_rx() {
    // Read all available characters in this interrupt
     printf("On UART");
    while (uart_is_readable(UART_ID)) {
        char ch = uart_getc(UART_ID);
        ...
        if (msg_index < MSG_BUFFER_SIZE - 1) {
            msg_buffer[msg_index++] = ch;
            
            // Check for end of message (newline)
            if (ch == '\n') {
                msg_buffer[msg_index] = '\0';  // Null terminate
                message_ready = true;
...
int main() {
    ...
    while (true){
            ...
            if (message_ready) {
            printf("Complete message received: %s", msg_buffer);
            message_ready = false;
            
            // Optionally send a response
            // uart_puts(UART_ID, "Message received!\n");
        }
